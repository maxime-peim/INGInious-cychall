 ...
 */
function studio_display_template_submit_message(
  title,
  content,
  type,
  dismissible
...
function studio_template_submit() {
  if (studio_template_submitting) return;
    ...
    if (path in studio_file_editor_tabs) {
      jQuery.ajax({
        success: function (data) {
          if ("error" in data)
            error +=
              "<li>An error occurred while saving the file " + path + "</li>";
  ...
  if (error)
    studio_display_template_submit_message(
      "Some error(s) occurred when saving the template: <ul>" + error + "</ul>",
      "",
      "danger",
      true
    );
...
function check_template_id(event) {
  ...
  if (!template_id.match(/^[a-zA-Z0-9_\-]+$/)) {
    alert(
      'Template id should only contain alphanumeric characters (in addition to "_" and "-").'
    );
...
function reload_options(problem_id) {
  ...
  studio_update_exercise_options({
